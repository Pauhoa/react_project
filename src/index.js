...
class App extends Component {
  userSchema = Yup.object().shape({
    ...
    items: Yup.array().of(Yup.object().shape({
      ...
      quantity: Yup.number().typeError('Doit être un nombre').min(5, 'Nombre trop petit')
    }))
  });
        ...
        >
          {({ handleSubmit, isSubmitting, values }) => (
            ...
            >
                    ...
                    {values.items && values.items.length
                      ? values.items.map((item, index) => (
                          <div key={ index }>
                            ...
                            <button type="button" className="w-100 btn btn-small btn-danger" onClick={ () => arrayHelpers.remove(index)}>delete</button>
                          </div>
                        ))
                      : null}
                      <button type="button" className="mt-3 w-100 btn btn-small btn-success" onClick={ () => arrayHelpers.push({
                        name: '',
                        quantity: 0
                      }) }>Add item</button>
                  </div>
              ...
              <button type="submit" className="mt-3 btn btn-primary" disabled={isSubmitting}>Submit</button>
            </form>
          )}
        </Formik>
      </div>
